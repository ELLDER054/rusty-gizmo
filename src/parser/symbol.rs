...
/// Stores information for each argument
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct Argument {
    ...
    /// Identifier counter for code generation
    pub id_c: usize,
}
...
/// Stores information for each array symbol
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct ArrSymbol {
...
pub struct Scope {
    ...
    pub array_symbols: Vec<ArrSymbol>,
}
...
impl SymbolController {
    ...
    pub fn add_symbol(&mut self, id: String, typ: String, symtyp: SymbolType, gen_id: String, length: Option<usize>, arguments: Option<Vec<Argument>>) {
        ...
        match symtyp.clone() {
            ...
            SymbolType::Func   => self.current.func_symbols.push(FuncSymbol {id: id, typ: typ, gen_id: gen_id, arguments: arguments.unwrap_or(Vec::new())}),
            SymbolType::Struct => self.current.struct_symbols.push(StructSymbol {id: id, gen_id: gen_id, entries: arguments.unwrap_or(Vec::new())}),
            SymbolType::Arr => self.current.array_symbols.push(ArrSymbol {id: id, typ: typ, len: length.unwrap_or(0), gen_id: gen_id}),
        }
    }

    /// Adds a scope to the symbol table
    pub fn add_scope(&mut self) {
        let new = Scope {parent: Some(Box::new(self.current.clone())), children: Vec::new(), var_symbols: Vec::new(), func_symbols: Vec::new(), struct_symbols: Vec::new(), array_symbols: Vec::new()};
    ...
    pub fn find(&self, id: String, symtyp: SymbolType) -> bool {
        ...
        match symtyp {
            ...
            SymbolType::Arr => {
                for sym in self.current.array_symbols.iter() {
                    if sym.id == id {
                        return true;
                    }
                }
    ...
    pub fn find_global_var_error(&self, id: String, token: &Token) -> VarSymbol {
        ...
        if sym == None {
            ...
            let helper = if autoc(id.clone(), cur_var_ids.clone()) == id.clone() {
                "You may have spelled this wrong".to_string()
            } else {
                format!("Maybe you meant '{}'", autoc(id.clone(), cur_var_ids))
            };
    ...
    pub fn find_global_func_error(&self, id: String, token: &Token) -> FuncSymbol {
        ...
        if sym == None {
            ...
            let helper = if autoc(id.clone(), cur_func_ids.clone()) == id.clone() {
                ...
                format!("Maybe you meant '{}'", autoc(id.clone(), cur_func_ids))
            };
            error(ErrorType::UndefinedSymbol, token)
                .note(format!("Undefined symbol '{}'", id).as_str())
                .help(helper.as_str())
    ...
    pub fn find_global_struct_error(&self, id: String, token: &Token) -> StructSymbol {
        ...
        if sym == None {
            ...
            let helper = if autoc(id.clone(), cur_struct_ids.clone()) == id.clone() {
                ...
                format!("Did you mean '{}'", autoc(id.clone(), cur_struct_ids))
            };
            error(ErrorType::UndefinedSymbol, token)
